<template>
	...
	<!-- <uni-search-bar radius="100" placeholder="输入查找的地址" clearButton="none" cancelButton="none" @confirm="search" /> -->
	<view class="select-city-wrap">
		...
		<!-- 	<view class="index-bg-view">
			...
			<scroll-view :scroll-into-view="scrollIntoId" @scroll="scrollChange" :scroll-y="false" :scroll-with-animation="isAnimation" :style="{ height: windowHeight,position:absolute }">
				<!-- <view class="content"> -->
						...
						<view class="city-list">
							<view  @click="onSelect(value)" class="city-item" style="display: flex; justify-content: center; align-items: center;">
								<image
									...
									src="../../../static/img/choose-city/system-map2.png"
									mode="aspectFit"
								></image>
								{{ current }}
							</view>
				...
				<!-- 	<view class="section" :id="item.letter" v-for="item in citys" :key="item.letter">
					...
					<view class="section"   >
						...
						<view class="city-title">其他城市</view>
						<view class="city-list">
							<view class="city-item" :class="{ active: current === city }" v-for="(city, itemIndex) in citysd.list" :key="itemIndex" @click="onSelect(city)">
								{{ city }}
							</view>
						</view>
					</view>
